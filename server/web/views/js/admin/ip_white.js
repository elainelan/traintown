app.controller('admin.ip_white.ctrl', function ($scope, focus, $rootScope, $http) {

    var api_name = 'admin.ip_white';
     // 生成面包屑导航
    ...
    $scope.modal_del = function(obj) {
        if (!$scope.is_moded()) {
            // 弹出删除确认框
            $rootScope.langs.modal_del_body = $rootScope.langs.confirm_del_white_ip;
            $scope.delobj = obj;
    ...
    $scope.ext_search_load = function() {
        if (typeof($scope.search_ext_loaded) == 'undefined') {
            ...
            {
                search_white_ip: {
                    message: $rootScope.langs.search_white_ip_invalid,
                    validators: {
                        regexp: {
